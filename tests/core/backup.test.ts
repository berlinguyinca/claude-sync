...
describe("core/backup", () => {
	...
	describe("createBackup", () => {
		it("copies only allowlisted files to timestamped directory", async () => {
			...
			await fs.writeFile(path.join(claudeDir, "projects", "foo.json"), "{}");

			const backupDir = await createBackup(claudeDir, backupBaseDir);

			// Allowlisted files should be present
			const claudeMd = await fs.readFile(path.join(backupDir, "CLAUDE.md"), "utf-8");
			expect(claudeMd).toBe("# Claude config");

			const settingsJson = await fs.readFile(path.join(backupDir, "settings.json"), "utf-8");
			expect(settingsJson).toBe('{"key":"value"}');

			// Non-allowlisted file should NOT be present
			await expect(fs.access(path.join(backupDir, "projects", "foo.json"))).rejects.toThrow();
		});
		...
		it("preserves directory structure in backup", async () => {
			...
			const agentFile = await fs.readFile(path.join(backupDir, "agents", "default.md"), "utf-8");
		...
		it("throws if claudeDir does not exist", async () => {
			...
			await expect(createBackup(nonExistent, backupBaseDir)).rejects.toThrow();
		});
